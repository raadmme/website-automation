...
export const SITE_SPEC_SCHEMA = {
  ...
  properties: {
    ...
    menu: {
      type: "array",
      description: "Menu or price list. Include ONLY for food, drink, or salon businesses.",
      items: {
        type: "object",
        properties: {
          name: { type: "string" },
          description: { type: "string" },
          price: { type: "string", description: "Display price, e.g. \"$12\"" }
        },
    ...
    team: {
      type: "array",
      description: "Team member profiles. Include ONLY for professional-service businesses where the people matter (law, accounting, medicine, consulting).",
      items: {
        type: "object",
        properties: {
          name: { type: "string" },
    ...
    serviceArea: {
      type: "string",
      description: "Geographic area served, e.g. \"Serving Portland and surrounding areas\". Include ONLY for local trades or delivery businesses."
    },
    bookingUrl: {
      type: "string",
      description: "Online booking link. Include ONLY if one appears in the input."
    }
  },
  required: [
...
- Optional sections — include ONLY where they genuinely fit the business:
  "menu" for restaurants, cafes, bakeries, bars, or salons (use prices from
  the input, or plausible modest placeholders); "team" for professional
  services where the individuals matter; "serviceArea" for local trades or
  delivery businesses; "bookingUrl" only if a booking link appears in the
  input. Omit these properties entirely otherwise.`;

/**
 * Generate a site spec from a business description, optional imported document
 * text, and optional images (e.g. a photographed flyer or menu).
 * images: [{ mediaType, data }] where data is base64.
 * Uses the Claude API when ANTHROPIC_API_KEY is set; otherwise falls back to
 * demo mode (which ignores images).
 */
export async function generateSiteSpec(description, documentText = "", images = []) {
  ...
  const content = [
    ...
    })),
    {
      type: "text",
      text: images.length
        ? `${userText}\n\nThe attached images are existing materials from the business (flyers, menus, signage, photos). Use any facts, offerings, and prices visible in them.`
        : userText
    }
  ];
...
/**
 * Revise a whole site spec from a conversational instruction, e.g.
 * "make the tone friendlier and add a service about gift baskets".
 * Requires a valid ANTHROPIC_API_KEY.
 */
export async function reviseSpec(spec, message) {
  if (!process.env.ANTHROPIC_API_KEY) {
    const err = new Error("Conversational revision requires an ANTHROPIC_API_KEY.");
    err.status = 400;
    throw err;
  }
  ...
  const client = new Anthropic();
  const prompt =
    `Here is the current content of a small-business website as JSON:\n` +
    `${JSON.stringify(spec, null, 2)}\n\n` +
    `Apply the following change requested by the site owner, keeping everything ` +
    `else intact and consistent with the tone-of-voice rules:\n\n${message}`;
  ...
  try {
    ...
    if (err instanceof Anthropic.AuthenticationError) {
      const friendly = new Error("Conversational revision requires a valid ANTHROPIC_API_KEY.");
      friendly.status = 400;
      throw friendly;
    }
    throw err;
...
export const REGENERATABLE_SECTIONS = [
  "tagline", "hero", "about", "services", "testimonials", "faq"
];

/**
 * Regenerate a single section of an existing site spec, optionally guided by
 * user instructions. Requires a valid ANTHROPIC_API_KEY.
 */
export async function regenerateSection(spec, section, instructions = "") {
  if (!REGENERATABLE_SECTIONS.includes(section)) {
    throw new Error(`Unknown section: ${section}`);
  }
  if (!process.env.ANTHROPIC_API_KEY) {
    const err = new Error("Section regeneration requires an ANTHROPIC_API_KEY.");
    err.status = 400;
    throw err;
  }

  const client = new Anthropic();
  const sectionSchema = SITE_SPEC_SCHEMA.properties[section];
  const schema = {
    type: "object",
    properties: { value: sectionSchema },
    required: ["value"],
    additionalProperties: false
  };
  ...
  const prompt =
    `Here is the current content of a small-business website as JSON:\n` +
    `${JSON.stringify(spec, null, 2)}\n\n` +
    `Rewrite ONLY the "${section}" section. Keep it consistent with the rest ` +
    `of the site and the tone-of-voice rules.` +
  ...
  try {
    ...
    if (err instanceof Anthropic.AuthenticationError) {
      const friendly = new Error("Section regeneration requires a valid ANTHROPIC_API_KEY.");
      friendly.status = 400;
      throw friendly;
    }
    throw err;
